...
export class PlatformizedWixDataProvider {
    ...
    _unwrapDataItem(response) {
        if (response && response.hasOwnProperty('dataItem') && response.dataItem.hasOwnProperty('data')) {
            return {
            ...
            .then(useDataItemService => {
                ...
                if (useDataItemService) {
                    return this._call('/v2/bulk/items/remove', {
                        ...
                        const errors = results.filter(result => !result.itemMetadata.success && result.itemMetadata.error.code !== 'WDE0073');
                ...
                if (useDataItemService) {
                    return this._call('/v2/items/query', {
                        ...
                        const isPartialIncludes = !!(items.find(item => {
                            return !!(isArray(include) && include.find(includedField => {
                ...
                if (useDataItemService) {
                    return this._call('/v2/items/aggregate', {
                        ...
                        const items = results.map(item => {
                            ...
                            if (typeof item._id === 'object' && Object.keys(item._id).length === 1) {
    ...
    _sortingFromOrder(order, fieldName) {
        const maybySortForField = order.find(o => o.hasOwnProperty(fieldName));
        const sorting = maybySortForField && maybySortForField.hasOwnProperty(fieldName) ? maybySortForField[fieldName] : undefined;
        ...
        return this._traceWith('platformized-provider-bulk-insert', {
            ...
            return apiValidator()
                ...
                .then(useDataItemService => {
                    ...
                    if (useDataItemService) {
                        const path = overrideExisting ? '/v2/bulk/items/save' : '/v2/bulk/items/insert';
                        return this._call(path, {
                            ...
                            const errors = results.filter(result => !result.itemMetadata.success && result.itemMetadata.error.code !== 'WDE0074');
                            return {
                                insertedItemIds,
                                inserted: inserted,
                ...
                if (useDataItemService) {
                    return this._call('/v2/bulk/items/save', {
                        ...
                        return {
                            ...
                            skipped: items.length - insertedItemIds.length - updatedItemIds.length - errors.length,
                ...
                .then(useDataItemService => {
                    if (useDataItemService) {
                        return this._call('/v2/items/query-referenced', {
                            ...
                            returnTotalCount: !omitTotalCount,
                            option: this._optionsWithPluginsItemService(options)
                        }, true, true).then(({
                            results,
                            pagingMetadata
                        }) => {
    ...
    _call(path, body, preferGet = false, useItemService = false) {
        return Promise.all([this._getGridAppId(), this._getAuthHeader()]).then(([gridAppId, authHeader]) => {
            return this.traceWith('wix-data-api-call', {
                ...
                const data = useItemService ? serializeDates(Object.assign(body, {
