...
const filterMixin = (Base = class {}) => class extends genericFilterMixin(Base) {
    ...
    _validateCollectionName(query, originalQuery, operationName) {
        ...
        const [newInvalidArguments, valid] = new FilterValidator(operationName, query.invalidArguments, this.constructor, this.constructorName)
    ...
    _matchesUrlized(field, operand) {
        const [newInvalidArguments, valid] = this._filterValidator('._matchesUrlized')
            ...
            .validateAndAggregate();
        if (valid) {
            const newFilterTree = this._makeNewFilter(field, ...createMatchesOrInFilter(operand));
