...
export const doFetch = async (recordStore, fromIndex, length, byRefField) => {
    const fetchedItems = await recordStore(byRefField).fold(
        () => QueryResults.Empty(),
        service => service.externalApi.getRecords(fromIndex, length),
    )
    return fetchedItems.get()
}

export const doFetchMax = async (
    recordStore,
    fromIndex,
    length,
    byRefField,
...
export const setUserFilterInitialData = userFilterInitialData => ({
    type: actionTypes.SET_USER_INPUT_OPTION_VALUES,
