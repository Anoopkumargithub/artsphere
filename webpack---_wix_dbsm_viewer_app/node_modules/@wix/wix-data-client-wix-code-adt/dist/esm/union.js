...
const matchWith = variant => pattern => {
    ...
    throw new Error(`Variant "${variant.name}" not covered in pattern with keys [${patternKeys}].\nThis could mean you did not include all variants in your Union's matchWith function.`);
};
const variantFactory = (unionName, name, fn, derivedMethods) => {
    ...
    variantCtor.hasInstance = variantInstance => variantInstance && variantInstance[symbol] === true;
    return variantCtor;
};
const union = (unionName, variants, derivedMethods = {}) => {
    return Object.keys(variants).reduce((acc, variantName) => {
        acc[variantName] = variantFactory(unionName, variantName, variants[variantName], derivedMethods);
        return acc;
    }, {});
};
union.any = any;
