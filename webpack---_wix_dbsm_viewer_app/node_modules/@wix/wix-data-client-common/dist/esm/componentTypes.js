...
export default mapValues(types, ({
    sdkType
}) => sdkType);
export const getSdkTypeByViewerType = (viewerType) => Maybe.fromNullable(keys(types)
    .filter(key => types[key].viewerType.indexOf(viewerType) >= 0)
    .map(key => types[key].sdkType)[0]);
export const getViewerTypeBySdkType = (sdkType) => Maybe.fromNullable(keys(types)
    .filter(key => types[key].sdkType === sdkType)
    .map(key => types[key].viewerType[0])[0]);
export const getViewerTypes = keys(types).reduce((acc, val) => {
    acc.concat(types[val].viewerType);
    return acc;
}, []);
export const viewerTypes = keys(types)
...
export const sdkTypes = keys(types).map(key => types[key].sdkType);
export const isSdkTypeSupported = (sdkType) => !!mapKeys(types, ({
    sdkType
}) => sdkType)[sdkType];
